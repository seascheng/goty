...
extension Ghostty {
    ...
    class SurfaceView: OSView, ObservableObject, Identifiable {
        ...
        /// GOTY: invoked on renderer UPDATE_FRAME_END (renderer thread);
        ...
        deinit {
            ...
            // GOTY: UNUserNotificationCenter requires a bundle proxy,
        ...
        /// Apply a daemon stream's ordered grid marker. Unlike
        /// `sizeDidChange`, this does not derive a grid from the view frame:
        /// replay segments carry the geometry their bytes were produced at.
        /// Keeping `surfaceSize` in sync is required by SurfaceScrollView and
        /// PaneGrid; bypassing the Swift cache leaves scrollbars and layout on
        /// the previous grid even though the core already moved.
        func setGridSize(columns: UInt16, rows: UInt16) -> ghostty_surface_size_s? {
            guard let surface else { return nil }
            var resolved = ghostty_surface_size_s()
            guard ghostty_surface_set_grid_size(surface, columns, rows, &resolved) else {
                return nil
            }
            // goty: grid markers are applied on the pane's stream
            // queue (never main); Published properties are main-only —
            // same DispatchQueue pattern as setSurfaceSize above.
            DispatchQueue.main.async { self.surfaceSize = resolved }
            return resolved
        }
        ...
        override func viewDidChangeBackingProperties() {
            ...
            // goty: this resync also runs at attach time, when the
            // frame is still zero — 0/0 is NaN, and a NaN content scale
            // poisons the core's font metrics (mixed-DPI tiny/huge text
            // until the next REAL backing change, i.e. moving screens).
            // Derive the scale from the window whenever the frame can't
            // provide it, and never push a non-finite or empty size.
            let frame = self.frame
            var xScale = window?.backingScaleFactor ?? 2
            var yScale = xScale
            if frame.size.width > 0, frame.size.height > 0 {
                let fbFrame = self.convertToBacking(frame)
                xScale = fbFrame.size.width / frame.size.width
                yScale = fbFrame.size.height / frame.size.height
            }
            guard xScale.isFinite, xScale > 0, yScale.isFinite, yScale > 0 else { return }
            ghostty_surface_set_content_scale(surface, xScale, yScale)

            // When our scale factor changes, so does our fb size so we send that too
            let scaledSize = self.convertToBacking(contentSize)
            if scaledSize.width.isFinite, scaledSize.width > 0,
               scaledSize.height.isFinite, scaledSize.height > 0 {
                setSurfaceSize(width: UInt32(scaledSize.width), height: UInt32(scaledSize.height))
            }
        }
        ...
        override func mouseMoved(with event: NSEvent) {
            ...
            // GOTY: focus-follows-mouse requires the upstream
        ...
        @IBAction func copy(_ sender: Any?) {
            ...
            if !ghostty_surface_binding_action(surface, action, UInt(action.lengthOfBytes(using: .utf8))) {
                AppLog.warning("action failed action=\(action)")
            }
        }
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            UNUserNotificationCenter.current().add(request) { @MainActor error in
                if let error = error {
                    AppLog.error("Error scheduling user notification: \(error)")
                    return
                }
