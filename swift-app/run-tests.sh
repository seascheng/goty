...
# settingstest pops the real Settings window and webuitest drives the
# agent GUI in a real WKWebView — window-opening tests are off until
# they can run without landing windows on the user's screen. The files
# stay in tools/ for when they come back.
FAILED=""
for t in layouttest filestest aitest agenttest; do
